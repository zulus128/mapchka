...
static SEL downloadProgressCustomSelector;
static SEL downloadViewProgressCustomSelector;
// Private stuff
...
- (void)downloadProgressCall:(NSString *)str{
    if([downloadProgressDelegateReal respondsToSelector:downloadProgressCustomSelector])
        [downloadProgressDelegateReal performSelector:downloadProgressCustomSelector withObject:str withObject:self];
}

- (void)setProgress:(float)_progress{
    if([downloadProgressDelegateReal respondsToSelector:downloadViewProgressCustomSelector])
        [downloadProgressDelegateReal performSelector:downloadViewProgressCustomSelector
                                           withObject:[NSNumber numberWithFloat:_progress]
                                           withObject:self];
}
...
- (id)initWithURL:(NSURL *)newURL
{
    ...
    [ASIHTTPRequest setDownloadProgressCustomSelector:@selector(downloadedProgress:request:)];
    [ASIHTTPRequest setDownloadViewProgressCustomSelector:@selector(setProgress:forRequest:)];
...
- (void)startAsynchronousWithcompletion:(RequestHandlerBlock)handler{
...
- (void)startAsynchronous
{
    ...
    if(self.queuePriority == NSOperationQueuePriorityVeryHigh)
...
{
	[[self cancelledLock] lock];
	id d = [[downloadProgressDelegateReal retain] autorelease];
	[[self cancelledLock] unlock];
	return d;
}
...
{
	[[self cancelledLock] lock];
	downloadProgressDelegateReal = newDelegate;
    downloadProgressDelegate = downloadProgressDelegateReal ? self : nil;
...
#define bytePrefixPowerCount 8
+(NSString*)byteSizeToString:(unsigned long long)_byteSize
{
	int k=0;
    double byteSize = (double)_byteSize;
    
	while (byteSize>1024 && k<bytePrefixPowerCount)
	{
	...
	switch (k)
	{
		...
		case 5:		prefix=@"ПБ";	break;
		case 6:		prefix=@"ЭБ";	break;
		case 7:		prefix=@"ЗБ";	break;
		default:	prefix=@"ЙБ";	break;
	}
	return [[NSString stringWithFormat:(k?@"%.2f %@":@"%.0f %@"),byteSize,prefix] stringByReplacingOccurrencesOfString:@"." withString:@","];
}


+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
    if(total < 200 || progress - total < 200)
        return;
#if TARGET_OS_IPHONE
    ...
    if (![*indicator respondsToSelector:selector] && ![*indicator respondsToSelector:@selector(downloadProgressCall:)])
        return;
    ...
    if([*indicator respondsToSelector:@selector(downloadProgressCall:)]){
        
        NSString *str = [NSString stringWithFormat:@"%0.2f%%", progressAmount / 3.0 * 100.0];
        //                         [ASIHTTPRequest byteSizeToString:progress],
        //                         [ASIHTTPRequest byteSizeToString:total]] ;
        [ASIHTTPRequest performSelector:@selector(downloadProgressCall:)
                               onTarget:indicator
                             withObject:str
                                 amount:nil
                         callerToRetain:str];
...
- (void)requestFinished
{
    ...
    BOOL found = NO;
    for (NSOperation *oper in sharedQueue.operations) {
        if(!oper.isExecuting && oper.queuePriority == NSOperationQueuePriorityVeryHigh){
            sharedQueue.maxConcurrentOperationCount += 1;
            found = YES;
            break;
        }
...
- (void)reportFinished
{
    if(self.handlerBlock){
        self.handlerBlock(self, self.responseData, self.error);
    }
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
                ...
                if(![manager fileExistsAtPath:[[self downloadDestinationPath] stringByDeletingLastPathComponent]]){
                    [manager createDirectoryAtPath:[[self downloadDestinationPath] stringByDeletingLastPathComponent]
                       withIntermediateDirectories:YES
                                        attributes:nil
                                             error:&moveError];
                }
				[manager moveItemAtPath:[self temporaryUncompressedDataDownloadPath] toPath:[self downloadDestinationPath] error:&moveError];
				if (moveError) {
			...
			if (!fileError) {
				...
				[manager moveItemAtPath:[self temporaryFileDownloadPath] toPath:[self downloadDestinationPath] error:&moveError];
				if (moveError) {
					fileError = [NSError errorWithDomain:NetworkRequestErrorDomain code:ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    newRequest.handlerBlock = self.handlerBlock;
...
+ (void)setDownloadViewProgressCustomSelector:(SEL)sel{
    downloadViewProgressCustomSelector = sel;
}

+ (void)setDownloadProgressCustomSelector:(SEL)sel{
    downloadProgressCustomSelector = sel;
}
