 ...
 rightButtonAction:(void (^)(void)) rightButtonAction
{
               ...
               otherButtonTitles:rightButtonTitle, nil]))
  {
    ...
    if (leftButtonAction) 
    {
      objc_setAssociatedObject(self, CFBridgingRetain(LEFT_ACTION_ASS_KEY), leftButtonAction, OBJC_ASSOCIATION_COPY_NONATOMIC);
    }
    
    if (rightButtonAction) 
    {
      objc_setAssociatedObject(self, CFBridgingRetain(RIGHT_ACTION_ASS_KEY), rightButtonAction, OBJC_ASSOCIATION_COPY_NONATOMIC);
    }
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
  ...
  if (buttonIndex == 0) 
  {
    action  = objc_getAssociatedObject(self, CFBridgingRetain(LEFT_ACTION_ASS_KEY));
  } 
  else if (buttonIndex == 1)
  {
    action  = objc_getAssociatedObject(self, CFBridgingRetain(RIGHT_ACTION_ASS_KEY));
  }
  ...
  objc_setAssociatedObject(self, CFBridgingRetain(LEFT_ACTION_ASS_KEY), nil, OBJC_ASSOCIATION_COPY);
  objc_setAssociatedObject(self, CFBridgingRetain(RIGHT_ACTION_ASS_KEY), nil, OBJC_ASSOCIATION_COPY);
